...
(function (win, jquery, _) {
  ...
  win.booktype.editor = (function () {
    ...
    var enableEditor = function (){
      ...
      if (data.editorHTML){
        Aloha.getEditableById('contenteditor').setContents(data.editorHTML);
        data.editorHTML = null; 
    ...
    var disableEditor = function (){
      ...
      if (!data.editorHTML){
        data.editorHTML = Aloha.getEditableById('contenteditor').getContents();
      }
    };
          ...
          jquery(v.domID).click(function () {
            ...
            if (jquery('#right-tabpane').hasClass('open')) {
              ...
              if (jquery(this).closest('li').hasClass('active')) {
                jquery(document).trigger('booktype-tab-deactivate', [v.tabID]);
    ...
    var _initUI = function () {
      ...
      activateTabs(tabs);
      jquery(document).on('booktype-tab-activate', function (event, newActiveTab, oldActiveTab){
        switch (oldActiveTab){
          case 'online-users-tab':
            t1.onDeactivate();
            break;
          case 'chat-tab':
            t2.onDeactivate();
        ...
        window.booktype.currentActiveTab = newActiveTab;
      });
    ...
    var _loadChapterHistory = function (){
      win.booktype.sendToCurrentBook({
        ...
        var restore_label = win.booktype._('restore', 'RESTORE');
        ...
        jquery('#history-tab-list li').click(function (){
          ...
          win.booktype.sendToCurrentBook({
            ...
            'chapter': currentEdit,
            'revision1': jquery(this).find('button')[0].dataset.revision, 
      ...
      win.booktype.subscribeToChannel('/chat/' + win.booktype.currentBookID + '/',
        function (message) {
          ...
          if (message.command === 'message_info') {
            if (message.message_id === 'user_saved_chapter'){
              _loadChapterHistory();
            }
            if (tabs[1]) {
